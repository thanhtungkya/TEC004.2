...
NHADATVIET123_URL = 'https://123nhadatviet.com/rao-vat/can-ban/nha-dat/t1/ha-noi.html'

def scrape_nhadatviet123(progress_cb=None, log_cb=None, abort_event=None):
    records = []
    ...
    for item in cards:
        ...
        try:
            ...
            'listing_date': listing_date, 'source': '123nhadatviet', 'url': url,
        })
            if progress_cb:
                progress_cb('nhadatviet123')
            if log_cb:
                log_cb('nhadatviet123', 'Success', url)
        except Exception as exc:
            if log_cb:
                log_cb('nhadatviet123', 'Fail', f"{url} - {exc}")
    return records[:200]
